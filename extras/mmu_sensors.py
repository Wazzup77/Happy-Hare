...
class MmuRunoutHelper:
    ...
    def _handle_ready(self):
        ...
        )  # Time to wait before first events are processed

    def _insert_event_handler(self, eventtime):
        insert_gcode = self.gcodes.get("insert")
        self._exec_gcode(
            "%s EVENTTIME=%s" % (insert_gcode, eventtime) if insert_gcode else None
        )

    def _remove_event_handler(self, eventtime):
        remove_gcode = self.gcodes.get("remove")
        self._exec_gcode(
            "%s EVENTTIME=%s" % (remove_gcode, eventtime) if remove_gcode else None
        )
    ...
    def _runout_event_handler(self, eventtime, event_type):
        ...
        self._exec_gcode(
            "%s EVENTTIME=%s" % (handler_gcode, eventtime) if handler_gcode else None
        )
    ...
    def _exec_gcode(self, command):
        if command:
            try:
                ...
                logging.exception(
                    "MMU: Error running mmu sensor handler: `%s`" % command
                )
    ...
    def _process_state_change(self, eventtime, is_filament_present):
        ...
        if print_stats is not None:
            ...
            is_printing = (
                self.printer.lookup_object("idle_timeout").get_status(now)["state"]
                == "Printing"
            )
        ...
        if is_filament_present and insert_gcode:  # Insert detected
            if not is_printing or (is_printing and self.insert_remove_in_print):
                ...
                self.min_event_systime = (
                    self.reactor.NEVER
                )  # Prevent more callbacks until this one is complete
                self.reactor.register_callback(
                    lambda reh: self._insert_event_handler(eventtime)
                )
            ...
            if is_printing and self.runout_suspended is False and runout_gcode:
                ...
                self.reactor.register_callback(
                    lambda reh: self._remove_event_handler(eventtime)
                )

    def note_clog_tangle(self, event_type, eventtime=None):
        ...
        self.reactor.register_callback(
            lambda reh: self._runout_event_handler(eventtime, event_type)
        )
...
class MmuProportionalSensor:
    def __init__(self, config, name):
        ...
        gcodes = {"clog": clog_gcode, "tangle": tangle_gcode}
...
class MmuAdcSwitchSensor(MmuAdcSensorBase):
    def __init__(
        ...
        insert_gcode = (
            (
                "%s SENSOR=%s%s"
                % (INSERT_GCODE, name, (" GATE=%d" % gate) if gate is not None else "")
            )
            if insert
            else None
        )
        remove_gcode = (
            (
                "%s SENSOR=%s%s"
                % (REMOVE_GCODE, name, (" GATE=%d" % gate) if gate is not None else "")
            )
            if remove
            else None
        )
        runout_gcode = (
            (
                "%s SENSOR=%s%s"
                % (RUNOUT_GCODE, name, (" GATE=%d" % gate) if gate is not None else "")
            )
            if runout
            else None
        )
    ...
    def adc_callback(self, read_time, read_value):
        ...
        if (
            (read_time - self.last_debouncetime) >= self.adc_debounce_time
            ...
            if (
                self.runout_helper.button_handler
                or is_present != self.runout_helper.filament_present
            ):
    ...
    def get_status(self, eventtime):
        ...
        status.update(
            {"Resistance": round(self.pullup * adc / (1.0 - adc)), "ADC": val}
        )
        return status
...
class MmuHallSensor(MmuAdcSensorBase):
    ...
    def adc_callback(self, read_time, read_value):
        ...
        if present != self.present:
            ...
            if (
                self.runout_helper.button_handler
                or present != self.runout_helper.filament_present
            ):
...
class MmuHallEndstop:
    def __init__(
        ...
        self.diameter = 0
        self.is_active = (
            True  # Always active for endstop purposes? or should be toggleable?
        )
        ...
        # We can't access self.adc here as it doesn't exist yet, so check available methods on a dummy pin or catch exception
        # Actually easier to just check hasattr on the created pin object

        # ADC 1
        if self.pin1_name:
            ppins.allow_multi_use_pin(self.pin1_name)
            self.mcu_adc = ppins.setup_pin("adc", self.pin1_name)
            if hasattr(self.mcu_adc, "setup_minmax"):  # Kalico and older klipper
        ...
        if self.pin2_name:
            ...
            if hasattr(self.mcu_adc2, "setup_minmax"):
        ...
        clog_gcode = ("%s SENSOR=%s" % (CLOG_GCODE, name)) if clog else None
        tangle_gcode = ("%s SENSOR=%s" % (TANGLE_GCODE, name)) if tangle else None
    ...
    def _calc_diameter(self):
        ...
        try:
            ...
            self.diameter = (
                2.0 * self.diameter + diameter_new
            ) / 3  # Slightly faster smoothing
...
class MmuSensors:
    def __init__(self, config):
        ...
        # Setup analog (proportional) sync feedback (Mainline feature)
        # Uses single analog input; value scaled in [-1, 1]
        analog_pin = config.get("sync_feedback_analog_pin", None)
        if analog_pin:
            self.sensors[Mmu.SENSOR_PROPORTIONAL] = MmuProportionalSensor(
                config, name=Mmu.SENSOR_PROPORTIONAL
            )

        # For Qidi printers or any other that use a hall_filament_width_sensor as an endstop (Mainline feature)
    ...
    def _create_sensor(
        ...
        switch_pins = (
            [switch_pins] if not isinstance(switch_pins, list) else switch_pins
        )
        switch_pins_2 = (
            [switch_pins_2]
            if switch_pins_2 and not isinstance(switch_pins_2, list)
            else (switch_pins_2 or [])
        )

        # Sanity checks for pin numbers
        if len(switch_pins) not in [1, self.num_units]:
            raise config.error(
                "Invalid number of pins specified for %s. Expected 1 or %d but counted %d"
                % (name_prefix, self.num_units, len(switch_pins))
            )
        if len(switch_pins_2) > 0 and len(switch_pins_2) != len(switch_pins):
            raise config.error(
                "Invalid number of secondary analog pins specified for hall sensor %s. Expected %d to match primary pins"
                % (name_prefix, len(switch_pins))
            )

        for unit, switch_pin in enumerate(switch_pins):
            if not self._is_empty_pin(switch_pin):
                name = (
                    "%s_%d" % (name_prefix, gate)
                    if gate is not None
                    else "unit_%d_%s" % (unit, name_prefix)
                    if len(switch_pins) > 1
                    else name_prefix
                )  # Must match mmu_sensor_manager
                switch_pin_2 = (
                    switch_pins_2[unit] if unit < len(switch_pins_2) else None
                )
                ...
                if analog_range is not None:
                    ...
                    if switch_pin_2 is not None:
                        ...
                        s = MmuHallSensor(
                            ...
                            adc_report_time=adc_report_time,
                        )
                        self.sensors[name] = s
                        ...
                        self.sensors[name] = s
                        logging.info(
                            "MMU: Added analog switch sensor to manager: %s" % name
                        )
                    else:
                        raise config.error(
                            "Invalid sensor definition for analog sensor %s. Missing pullup or secondary pin"
                            % name
                        )
                else:
                    # Standard switch sensor case
                    self._create_simple_switch_sensor(
                        config,
                        name,
                        gate,
                        switch_pin,
    ...
    def _create_simple_switch_sensor(
        self,
        config,
        name,
        gate,
    ...
    def _sync_tension_callback(self, eventtime, name, tension_state, runout_helper):
        ...
        compression_sensor = self.printer.lookup_object(
            "filament_switch_sensor %s_sensor" % Mmu.SENSOR_COMPRESSION, None
        )
        has_active_compression = (
            compression_sensor.runout_helper.sensor_enabled
            if compression_sensor
            else False
        )
        compression_state = (
            compression_sensor.runout_helper.filament_present
    ...
    def _sync_compression_callback(
        ...
        tension_sensor = self.printer.lookup_object(
            "filament_switch_sensor %s_sensor" % Mmu.SENSOR_TENSION, None
        )
        has_active_tension = (
            tension_sensor.runout_helper.sensor_enabled if tension_sensor else False
        )
        tension_state = (
            tension_sensor.runout_helper.filament_present
